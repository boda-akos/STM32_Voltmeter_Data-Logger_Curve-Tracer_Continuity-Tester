...
byte kdrv[]={PB8,PB7,PB5,PB3,PB4,PB6};
...
void setup(void) {
  ...
  for (byte i=0; i<6; i++) pinMode(kdrv[i],INPUT_PULLUP); 
  ...
  Timer3.pause();  dvmFrame(0);  
}

void loop() {
                      // stat 0x00-0x0f dvm ; 0x10-0x1f beeper ; 0x20-0x2f scanner
if (scn !=0xff) { byte r=scan(); 
                    //Serial.println(r);
                    if (r==0) {dvmFrame(r);stat = 0x00; if (tim4 == 100) tim4=1000; else tim4=100; } //voltmeter
                    if (r==2) {dvmFrame(r);stat = 0x10;} //beeper
                    if (r==1) {dvmFrame(r);stat = 0x20;} //scanner
                    if (r==3) {            if (stat == 0x00) {stat |= 0x02; stat &=~0x04;Timer3.resume(); tim1 = millis()+timout;} 
                                      else if (stat == 0x02) {stat |= 0x04; stat &=~0x02;Timer3.resume(); tim1 = millis()+timout;} 
                                      else if (stat == 0x04) {stat &= ~0x06; Timer3.pause();} //sound off
                                   } 
                    if (r==4) {if (stat&0x08) {stat &= ~0x08;tft.setCursor(8,90); tft.fillRect(7,90,115,30,ST7735_BLACK);}
...
if ((stat & 0xf0) == 0x00) { //============================ Voltmeter ================================
  ...
  if (stat & 0x08) { 
  dvmFrame(4); tft.print((float)tim4/1000,1); tft.print(" sec");
  while  (scan() != 0  ) { //wait for = sign
  ...
  while(scan() != 5 && scan() !=4 && scan() !=0 );  //react to + - = only
  if (scan() ==5 && nidx>50) nidx -=50;  if (scan() == 4 ) nidx +=50; if (nidx > 3600) nidx=3600;
...
if ((stat & 0xf0) == 0x20) {//============================ Scanner ===================================
 Timer3.pause();
  ...
  for (byte i=0; i<3; i++) { tft.setCursor(3, 113-i*40); tft.print(i);   } //dot line scale
...
byte scan(void){
  byte i,j,r=0xff;
  for (i=0; i<6; i++) {if (digitalRead(kdrv[i])==0) r=i;}
  return(r);
  }
...
void dvmFrame(byte x){tft.fillScreen(ST7735_BLACK); tft.drawRect(0,0,159,127,ST7735_BLUE);tft.drawRect(1,1,157,125,ST7735_BLUE);
                ...
                switch (x) {
                 case 0: { tft.print("Voltmeter"); break;}
                 case 2: { tft.print("Beeper"); tft.setTextSize(1);tft.setCursor(8,114); tft.print("Output 1mA"); break; }
                 case 1: { tft.setCursor(70,10);tft.print("Scanner"); break; } 
                 case 4: {tft.setTextSize(1);tft.print("Samples "); break;}
      }               
  }
